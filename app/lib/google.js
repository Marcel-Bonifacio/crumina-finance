...
async function gmailList(at,q,max=20){const r=await fetch('https://gmail.googleapis.com/gmail/v1/users/me/messages?q='+encodeURIComponent(q)+'&maxResults='+(parseInt(max,10)||20),{headers:{Authorization:'Bearer '+at}});if(!r.ok)throw new Error('gmail list '+r.status);return r.json();}
